...
; (function () {
	...
	function initialize(config_data, jsmo = null) {
		...
		if (config.isSurvey) {
			...
			if (config.progressBars.length) {
				orig_calculate = window['calculate'];
				window['calculate'] = function(t, isOnPageLoad) {
					orig_calculate(t, isOnPageLoad);
					updateProgressBars();
				};
			}
		}
	...
	function addProgressBars() {
		if (config.progressBars.length < 1) return;
		log('Adding progress bar(s):', config.progressBars);
		const $progress = $('<div class="n-unanswered-progress"></div>');
		const s = new Option().style;

		for (const bar of config.progressBars) {
			const id = 'n-unanswered-progress-' + bar?.calc;
			const $bar = $(`<div id="${id}" class="n-unanswered-bar"></div></div>`);
			s.color = bar?.color;
			if (bar.color && s.color === '') console.warn(`${config.progressAT}: Invalid color value:`, bar.color);
			s.height = bar?.height;
			if (bar.height && s.height === '') console.warn(`${config.progressAT}: Invalid height value:`, bar.height);
			if (s.color !== '') $bar.css('background-color', s.color);
			if (s.height !== '') $bar.css('height', s.height);
			$progress.append($bar);
		}
		$progress.insertBefore($('#form'));
	}

	function updateProgressBars() {
		if (config.progressBars.length < 1) return;
		log('Updating progress bar(s):', config.progressBars);

		for (const bar of config.progressBars) {
			const id = 'n-unanswered-progress-' + bar?.calc;
			const input = document.querySelector(`input[name="${bar?.calc}"]`);
			const val = Number.parseFloat(($(`input[name="${bar?.calc}"]`).val() ?? '0')?.toString());
	...
	function showDialog(dialogField, n, ob) {
		...
		const reenableButton = () => {
			if (config.isSurvey) $btn.button('enable');
		};
