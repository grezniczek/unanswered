...
class UnansweredExternalModule extends \ExternalModules\AbstractExternalModule
{
	...
	const AT_N_UNANSWERED_PROGRESS = '@N-UNANSWERED-PROGRESS';
	...
	private function inject_unanswered($context)
	{
		...
		$page_fieldnames = array_keys($page_fields);
		$add_progressbar = false;
		if ($context['is_survey']) {
			$tagged_progress = ActionTagHelper::getActionTags(self::AT_N_UNANSWERED_PROGRESS, $page_fieldnames, null, $context)[self::AT_N_UNANSWERED_PROGRESS] ?? [];
			$add_progressbar = count($tagged_progress) > 0;
		}
		$tagged = ActionTagHelper::getActionTags(self::AT_N_UNANSWERED, $page_fieldnames, null, $context)[self::AT_N_UNANSWERED] ?? [];
		if (!count($tagged) && !$add_progressbar) {
			return; // We are done
		}
		...
		if (!count($counters) && !$add_progressbar) {
		...
		$progressbars = [];
		if ($add_progressbar) {
			foreach ($tagged_progress as $fieldName => $pt) {
				$color = null;
				$height = null;
				$args = explode(',', trim($pt['params'], "\"'"));
				foreach ($args as $arg) {
					$arg = trim($arg);
					if (strlen($arg) === 0) continue;
					if (preg_match('/^\d+\s{0,1}(px|rem|em){0,1}$/', $arg)) {
						$height = intval($arg);
						if (!ends_with($height, 'px') || !ends_with($height, 'em')) {
							$height .= 'px';
						}
					}
					else {
		...
		$config = array(
			...
			'debug' => $this->js_debug,
			'counters' => $counters,
			'fields' => array_values(array_filter($page_fieldnames, function ($field_name) use ($counters, $page_fields, $instrument) { 
			...
			'progressAT' => self::AT_N_UNANSWERED_PROGRESS,
		);
